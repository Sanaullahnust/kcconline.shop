...
const ProductTemplateView: React.FC = () => {
  ...
  const oldPrice = Math.round((unitPrice * 1.45) / 100) * 100; // Dynamic discount anchor
  const totalPrice = unitPrice * quantity;

  // Visual feedback when capacity/price changes
  useEffect(() => {
    setPriceFlash(true);
  ...
  const handleAddToCart = () => {
    ...
    console.log("Conceptual Order Added to Cart:", orderDetails);
    setIsSuccess(true);
    setShowToast(true);
  ...
  const handleWhatsAppOrder = () => {
    const message = encodeURIComponent(`Hi KCC Online, I want to order:\n\nProduct: KCC Ultra-Fast Pro Power Station\nColor: ${selectedColor.name}\nCapacity: ${selectedCap.name}\nQuantity: ${quantity}\nTotal: Rs. ${totalPrice.toLocaleString()}\n\nPlease confirm availability.`);
    window.open(`https://wa.me/${businessNumber}?text=${message}`, '_blank');
  };

  const shareUrl = window.location.href;
  const shareText = "Check out this amazing KCC Ultra-Fast Pro Power Station!";
      ...
      <div className={`fixed top-32 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 pointer-events-none ${showToast ? 'translate-y-0 opacity-100' : '-translate-y-8 opacity-0'}`}>
        <div className="bg-emerald-600 text-white px-8 py-4 rounded-[2rem] shadow-2xl flex items-center gap-4 border border-emerald-400/20 backdrop-blur-md">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-bold text-sm">{selectedCap.name} - Added to Cart</span>
          </div>
        </div>
      </div>
            ...
            <div className="p-8 lg:p-12 space-y-8 bg-slate-50/50">
              <div className="relative aspect-square rounded-[3rem] overflow-hidden bg-white premium-shadow border border-slate-100 group">
                ...
                <img 
                  ...
                  className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-out transform group-hover:scale-110 ${fade ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}
                ...
                <div className="absolute top-10 left-10 flex flex-col gap-3">
                  <span className="bg-slate-900 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl flex items-center gap-2">
                    <ShieldCheck size={14} className="text-emerald-400" /> KCC Official Stock
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-4 justify-center">
                {variants.colors.map((c, i) => (
                  <button 
                    ...
                    className={`w-24 h-24 rounded-3xl overflow-hidden border-2 transition-all p-1 ${selectedColor.name === c.name ? 'border-emerald-600 ring-8 ring-emerald-50 shadow-xl' : 'border-white opacity-40 hover:opacity-100'}`}
                  >
                    <img src={c.img} className="w-full h-full object-cover rounded-2xl" alt={c.name} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="p-8 lg:p-20 flex flex-col justify-center space-y-12">
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] border-l border-slate-200 pl-4">Premium Series</span>
                </div>
                ...
                <p className="text-slate-500 text-lg leading-relaxed font-medium">
                  Optimized for Pakistani power conditions. Fast charge anything from earbuds to tablets with consistent voltage protection.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                  <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 inline-flex flex-col gap-1 min-w-[240px]">
                     <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em]">Limited Time Offer</span>
                     </div>
                  ...
                  <div className="space-y-3">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                       <Share2 size={12} /> Share this Product
                    </p>
                    <div className="flex gap-2">
                      <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" className="w-10 h-10 rounded-xl bg-[#1877F2]/10 text-[#1877F2] flex items-center justify-center hover:bg-[#1877F2] hover:text-white transition-all shadow-sm">
                        <Facebook size={18} />
                      </a>
                      <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareText)}`} target="_blank" className="w-10 h-10 rounded-xl bg-slate-900/10 text-slate-900 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                        <Twitter size={18} />
                      </a>
                    </div>
                  </div>
              ...
              <div className="space-y-10">
                <div className="space-y-6">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] block">Select Finish</label>
                  <div className="flex gap-4">
                    {variants.colors.map((color, i) => (
                      <button
                        key={i}
                        onClick={() => handleColorChange(color)}
                        className={`w-14 h-14 rounded-full border-2 p-1.5 transition-all relative ${selectedColor.name === color.name ? 'border-emerald-600 ring-4 ring-emerald-50' : 'border-slate-100 hover:border-slate-300'}`}
                      >
                        <div className="w-full h-full rounded-full shadow-inner border border-black/5" style={{ backgroundColor: color.hex }}></div>
                        {selectedColor.name === color.name && (
                          <div className="absolute -bottom-1 -right-1 bg-emerald-600 text-white rounded-full p-1 shadow-lg animate-in zoom-in">
                            <CheckCircle size={10} />
                          </div>
                        )}
                      </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {variants.capacities.map((cap, i) => (
                      <button
                        ...
                        className={`text-left p-6 rounded-3xl border-2 transition-all relative overflow-hidden group ${selectedCap.name === cap.name ? 'border-emerald-600 bg-emerald-50/30' : 'border-slate-100 hover:border-slate-200'}`}
                      >
                        <p className={`font-black text-sm uppercase tracking-tight mb-1 ${selectedCap.name === cap.name ? 'text-emerald-900' : 'text-slate-900'}`}>{cap.name}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{cap.desc}</p>
                        {cap.upcharge > 0 && (
                           <div className="absolute top-2 right-2 px-2 py-0.5 bg-slate-900 text-white text-[8px] font-black rounded-full">+Rs.{cap.upcharge.toLocaleString()}</div>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-6 pt-10 border-t border-slate-100">
                   ...
                   <div className="flex justify-between items-center px-2">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pricing Summary</span>
                      <span className="text-sm font-bold text-slate-900">
                        {quantity} units x Rs. {unitPrice.toLocaleString()}
                      </span>
                   </div>
                   ...
                   <div className="flex flex-col sm:flex-row items-center gap-6">
                      <div className="flex items-center gap-6 bg-slate-50 p-2 rounded-2xl border border-slate-100 w-full sm:w-fit justify-between">
                         <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-900 hover:bg-emerald-600 hover:text-white transition-all"><Minus size={18} /></button>
                         <span className="w-8 text-center font-black text-xl">{quantity}</span>
                         <button onClick={() => setQuantity(quantity + 1)} className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-900 hover:bg-emerald-600 hover:text-white transition-all"><Plus size={18} /></button>
                      </div>
                      
                      <button 
                        onClick={handleAddToCart}
                        disabled={isSuccess}
                        className={`flex-1 w-full py-6 rounded-3xl font-black text-lg shadow-2xl transition-all flex items-center justify-center gap-4 active:scale-95 group relative overflow-hidden ${isSuccess ? 'bg-emerald-600 text-white shadow-emerald-600/20' : 'bg-slate-900 text-white shadow-slate-900/20 hover:bg-black'}`}
                      >
                   ...
                   <button 
                     ...
                     className="w-full py-6 bg-emerald-600 text-white rounded-3xl font-black text-lg shadow-2xl shadow-emerald-600/30 hover:bg-emerald-700 transition-all flex items-center justify-center gap-4 active:scale-95 group"
                   >
              ...
              <div className="grid grid-cols-3 gap-4 pt-4">
                 {[
                   ...
                   { icon: <MapPin size={16} />, text: 'COD NATIONWIDE' },
                   { icon: <Package size={16} />, text: '7-DAY WARRANTY' },
                 ].map((sig, i) => (
